...
pub const SUSPEND_REASON_CODES: [&str; 4] =
    ["patient_not_ready", "elevator_outage", "urgent_interruption", SUSPEND_REASON_OTHER];
...
pub fn can_transition(current_status: &str, target_status: &str) -> bool {
    ...
    let allowed: &[&str] = match current {
        ...
        STATUS_ASSIGNED => &[STATUS_IN_PROGRESS, STATUS_SUSPENDED, STATUS_PENDING, STATUS_CANCELED],
        ...
        STATUS_SUSPENDED => &[STATUS_ASSIGNED, STATUS_IN_PROGRESS, STATUS_PENDING, STATUS_CANCELED],
...
pub fn normalize_reason_code(input: Option<&str>) -> Option<String> {
    input.map(|value| value.trim().to_ascii_lowercase()).filter(|value| !value.is_empty())
}
...
pub fn validate_cancellation_reason(
    ...
    if reason == CANCEL_REASON_OTHER {
        let has_comment = comment.map(|value| !value.trim().is_empty()).unwrap_or(false);
        if !has_comment {
